...
Type.prototype.validate = function (/* arg1, arg2, arg3, argN */) {
  ...
  for (var i = 0; i < arguments.length; i++) {
    if (rules.invalid.size && rules.invalid.has(arguments[i])) {
      return i;
    }

    if (rules.onlyValid) {
      if (rules.valid.size === 0 || !rules.valid.has(arguments[i])) {
        return i;
      }
    } else {
      if (!this._validate(arguments[i]) && (rules.valid.size === 0 || !rules.valid.has(arguments[i]))) {
        return i;
      }
    }
  }
